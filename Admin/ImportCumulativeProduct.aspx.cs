...
public partial class Admin_ImportCumulativeProduct : System.Web.UI.Page
{
    ...
    public String Get_CategoryId(string CategoryName)
    {
        if (CategoryName != "")
        {
            ...
            else
            {
                ...
                return Get_CategoryId(CategoryName);
            }
        }
        else
        {
        ...
        if (dt != null && dt.Rows.Count != 0)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (dr[0].ToString().Trim() != "")
                {
                    ...
                    //string dateString = dr[13].ToString().Trim();                  
                    //DateTime IPDate;
                    //string formats =  "dd/MM/yyyy";
                    //try {
                    //IPDate = Convert.ToDateTime(dateString, System.Globalization.CultureInfo.InvariantCulture);
                    //    IPDate = DateTime.ParseExact(dateString, "dd/MM/yyyy", new CultureInfo("en-US", true));
                    //}
                    //catch(Exception ex)
                    //{
                    //    double IP = double.Parse(dr[13].ToString());
                    //    DateTime IPDa = Convert.ToDateTime(DateTime.FromOADate(IP));
                    //    IPDate = Convert.ToDateTime(IPDa);
                    //}
                    //string inputString = dr[13].ToString();
                    //DateTime IPDate;
                    //if (DateTime.TryParse(inputString, out IPDate))
                    //{
                    //    String.Format("{dd/MM/yyyy}", IPDate);
                    //}
                    //else
                    //{
                    //    double IP = double.Parse(dr[13].ToString());
                    ...
                    else
                    {
                        ...
                        calldata = calldate.ToString();
                    }
                    i++;
                    if (MCategoryId != null && MCategoryId != "" && MPaymentType != "" && MPaymentType != null)
                    {
                        int IId = dl.add_ProductsBullet(null, MCategoryId, dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), dr[7].ToString(), putdata, calldata, dr[10].ToString(), Maturity.ToString("dd/MM/yyyy").Replace("-", "/"), MPaymentType, IPDate.ToString("dd/MM/yyyy").Replace("-", "/"), dr[14].ToString(), dr[15].ToString(), dr[16].ToString(), dr[17].ToString(), dr[18].ToString(), GetUserLoggedIn());
                        if (IId != 0)
                        {
            ...
            alertMsg.InnerHtml = "<div ><strong>Cumulative Product!</strong> " + msg + "</div>";
